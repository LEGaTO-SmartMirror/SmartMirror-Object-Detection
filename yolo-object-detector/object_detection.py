...
if __name__ == "__main__":
	...
	while True:
		...
		for det in dets:
			...
			wrel = w / darknet.network_width(netMain)
			hrel = h / darknet.network_height(netMain)
			detection_list.append( {"name": str(det[0].decode("utf-8")), "center": (float("{0:.5f}".format(xrel/horizontal_division )),float("{0:.5f}".format(yrel/vertical_division))),"box": (float("{0:.5f}".format(hrel)),float("{0:.5f}".format(wrel)))} )
		...
		to_node("detected",detection_list)
